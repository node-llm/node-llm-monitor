    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <TokenCard
          ...
          subValue={t('tokens.avgTokensPerReq', { count: formatTokenCount(totals.avgTokensPerRequest) })}
          icon="🎯"
          color="purple"
        />
        <TokenCard
          ...
          subValue={t('tokens.percentOfTotal', { percent: ((totals.totalPromptTokens / Math.max(totalTokens, 1)) * 100).toFixed(0) })}
          icon="📥"
          color="blue"
        />
        <TokenCard
          ...
          subValue={t('tokens.percentOfTotal', { percent: ((totals.totalCompletionTokens / Math.max(totalTokens, 1)) * 100).toFixed(0) })}
          icon="📤"
          color="green"
        />
        <TokenCard
          label={t('tokens.efficiencyRatio')}
          value={efficiency.toFixed(2)}
          subValue={t('tokens.outputInputRatio')}
          icon="⚡"
      ...
      <div className="glass rounded-2xl border border-monitor-border p-6">
        <h3 className="text-sm font-semibold text-gray-900 mb-4">📊 {t('tokens.tokenBreakdown')}</h3>
        <TokenBreakdownBar 
          promptTokens={totals.totalPromptTokens} 
          completionTokens={totals.totalCompletionTokens} 
          noDataLabel={t('tokens.noTokenData')}
        />
        <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-blue-500" />
            <span className="text-gray-600">{t('tokens.inputLabel')}: {formatTokenCount(totals.totalPromptTokens)}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-green-500" />
            <span className="text-gray-600">{t('tokens.outputLabel')}: {formatTokenCount(totals.totalCompletionTokens)}</span>
          </div>
        </div>
      </div>
      ...
      {byProvider.length > 0 && (
        <div className="glass rounded-2xl border border-monitor-border p-6">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">💰 {t('tokens.costByProvider')}</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-monitor-border">
                  <th className="text-left py-2 px-3 text-gray-500 font-medium">{t('metrics.providerModelHeader')}</th>
                  <th className="text-right py-2 px-3 text-gray-500 font-medium">{t('tokens.inputLabel')}</th>
                  <th className="text-right py-2 px-3 text-gray-500 font-medium">{t('tokens.outputLabel')}</th>
                  <th className="text-right py-2 px-3 text-gray-500 font-medium">{t('navigation.tokens')}</th>
                  <th className="text-right py-2 px-3 text-gray-500 font-medium">{t('metrics.costPer1kHeader')}</th>
                  <th className="text-right py-2 px-3 text-gray-500 font-medium">{t('metrics.efficiencyHeader')}</th>
                </tr>
              </thead>
              <tbody>
      ...
      <div className="glass rounded-2xl border border-monitor-border p-6 bg-gradient-to-r from-purple-50/50 to-blue-50/50">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">📈 {t('tokens.tokenSummary')}</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <p className="text-gray-500">{t('tokens.avgCostPer1k')}</p>
            <p className="text-lg font-semibold text-purple-600">${costPer1k.toFixed(4)}</p>
          </div>
          <div>
            <p className="text-gray-500">{t('tokens.tokensPerRequest')}</p>
            <p className="text-lg font-semibold text-blue-600">{formatTokenCount(totals.avgTokensPerRequest)}</p>
          </div>
          <div>
            <p className="text-gray-500">{t('dashboard.totalRequests')}</p>
            <p className="text-lg font-semibold text-gray-900">{totals.totalRequests}</p>
          </div>
          <div>
            <p className="text-gray-500">{t('dashboard.estCost')}</p>
...
function TokenBreakdownBar({ promptTokens, completionTokens, noDataLabel }: TokenBreakdownBarProps) {
  const total = promptTokens + completionTokens;
  if (total === 0) {
    return (
      <div className="h-8 bg-gray-100 rounded-full flex items-center justify-center text-sm text-gray-500">
        {noDataLabel || 'No token data'}
      </div>
    );
  }
